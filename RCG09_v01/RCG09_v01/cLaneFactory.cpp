...
cLane* cLaneFactory::create(cObject::ecType objectType, ecDirection direction, ecColor objectColor, int objectCount, int y, vector<int> time,
	int step, int crazyStep, int leftLimit, int rightLimit)
{
	switch (objectType)
	{
	case cObject::ecType::MV_CAR:
		return new cMovableObjectLane(objectType, direction, objectColor, objectCount, y, time, step, crazyStep, leftLimit, rightLimit);
